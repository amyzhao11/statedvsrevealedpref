...
for (iterate in 1:num){
  ...
  ndatesm<-round(rnorm(nsession,11.6875,1))
  ndatesf<-round(rnorm(nsession,11.6875,1))
...
write.csv(saved,paste("3101 Eastwick session",deparse(nsession),"traits",deparse(ntraits),"noise",deparse(noise),".csv"), row.names = FALSE)
  }
}
