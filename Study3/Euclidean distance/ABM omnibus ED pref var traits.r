...
setwd("C:/Users/s4357180/OneDrive - The University of Queensland/Writing/Pattern metric/Preference level ABM/Preference level/Omnibus/Euclidean distance")
...
noisetested=c(1,5,10,20,30,40,50)
...
calc <- function(a,noise,test=num_sims) {
  ...
  for (i in 1:test) {
    ...
    for (k in c(2, 3, 5, 9, 10, 15, 20, 25)) {
      ...
      a_i[[paste0("omniED", k)]]= 1*euclidean(a_i[,paste0("importance", 1:k)], (a_i[, paste0("rating", 1:k)]) )
      

      # Fit the linear mixed-effects model for each trait parameter
      model <- lmer(scale(get(paste0("attr", 25))) ~ 1 +
                      scale(get(paste0("omniED", k))) +
                      (1 | sessionID) +
                      (1 | participantID) +
  ...
  for (k in c(2, 3, 5, 9, 10, 15, 20, 25)) {
    write.csv(saved[[paste0(k)]], file = paste0("Omnibus ED pref 89 traits var traits", deparse(k), " noise ", deparse(noise), ".csv"))
  }
