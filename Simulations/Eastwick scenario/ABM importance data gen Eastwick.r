...
setwd(".../Data and code/Simulations/Eastwick scenario")
...
save(data,file=paste0("ABM Eastwick",deparse(nsession),"traits",deparse(ntraits),".RData"))
  }
#}
...
foreach (t = traitstested, .packages = packages) %dopar% {
  data=vector(mode = "list", length = num)
  abm(t)
}
...
noisetested=c(5,10)
#num=1000 #number of simulations
load("C:/Users/s4357180/OneDrive - The University of Queensland/Writing/Pattern metric/Submission 2/Code/ABM Eastwick 8 traits 10.RData")

#preprocess data for storage reasons
for (i in 1:1000){
  data[[i]]<-subset(data[[i]],select=c("participantID",paste0("importance ", 1:traits),
                                       "partnerID","sessionID","interact",paste0("rating ", 1:traits)))
}
...
calc <- function(a,noise,test=num_sims) {
  ...
  for (i in 1:test) {
    ...
    write.csv(saved[[paste0(k)]], file = paste0("Traitbytrait Eastwick traits ", deparse(k), " noise ", deparse(noise), ".csv"))
}
...
foreach (noisy = 10, .packages = packages) %dopar% {
  #data=vector(mode = "list", length = num)
  calc(data,noisy)
}
...
#summarise
a10_0=read.csv("Traitbytrait Eastwick traits 10 noise 0.csv")
a10_5=read.csv("Traitbytrait Eastwick traits 10 noise 5.csv")
a10_10=read.csv("Traitbytrait Eastwick traits 10 noise 10.csv")

a9_0=read.csv("Traitbytrait Eastwick traits 9 noise 0.csv")
a9_5=read.csv("Traitbytrait Eastwick traits 9 noise 5.csv")
a9_10=read.csv("Traitbytrait Eastwick traits 9 noise 10.csv")

a5_0=read.csv("Traitbytrait Eastwick traits 5 noise 0.csv")
a5_5=read.csv("Traitbytrait Eastwick traits 5 noise 5.csv")
a5_10=read.csv("Traitbytrait Eastwick traits 5 noise 10.csv")
sumdata=data.frame("traits"=c(rep(5,3),rep(9,3),rep(10,3)),
                   "noise"=rep(c(0,5,10),3),"estimate"=rep(NA,9),
                   "p"=rep(NA,9))

for (i in c(5,9,10)){
  for (j in c(0,5,10)){
    sumdata$estimate[sumdata$noise==j & sumdata$traits==i]= median(get(paste0("a",i,"_",j))$estimate)
    sumdata$p[sumdata$noise==j & sumdata$traits==i]=sum(as.numeric(get(paste0("a",i,"_",j))$p)<=0.05/i)
  }
}
